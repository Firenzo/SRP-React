(function() {
	...
	function Filters(props){
		...
		return (
			<form action="" id="directory-filters">
				<div className="group">
					...
					<input type="text" name="person_name" placeholder="Name of employee" id="txt-name" value={props.currentName} onChange={updateName}/>
				</div>
				<div className="group">
					<label htmlFor="person-title">Job Title:</label>
					<select name="person_title" id="sel-title" value={props.currentTitle} onChange={updateTitle}>
				...
				<div className="resetButton">
					<a className="button" onClick={props.resetFilter}>Reset</a>
				</div>
			</form>
		);
 	...
 	class Directory extends React.Component{ //ES6 Class
		...
		updateFormState(name, val){
			this.setState( {[name]: val}, this.updatePeopleList) //2e parameter is een callback functie die wordt uitgevoerd als setState klaar is
		}
